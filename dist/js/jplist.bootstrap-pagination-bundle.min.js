...
(b=this.name+this.options.delimiter0+"number="+a.data.number));return b};a.prototype.getStatusByDeepLink=function(b,a){var d;a:if(d=null,this.inDeepLinking){if("number"!==b&&b!=="path"+this.options.delimiter2+"type"+this.options.delimiter2+"order"&&"path"!==b){d=null;break a}d=e(this,!0);d.data&&"number"===b&&jQuery.isNumeric(d.data.number)&&(d.data.number=a)}return d};a.prototype.setStatus=function(a,c){var d=null,d="default",e=0;if(jQuery.isArray(a))for(var g=0;g<a.length;g++)a[g].data&&(a[g].data.path&&
(d=a[g].data.path),jQuery.isNumeric(a[g].data.number)||"all"===a[g].data.number)&&(e=a[g].data.number);else a.data&&a[g].data.path&&(jQuery.isNumeric(a.data.number)||"all"===a.data.number)&&(d=a.data.path,e=a.data.number);this.params.$buttons.attr("data-jplist-selected",!1);d="default"==d?f(this):"all"===e?this.params.$buttons.filter('[data-number="all"]'):this.params.$buttons.filter('[data-number="'+e+'"]');0<d.length&&(d.attr("data-jplist-selected",!0),this.params.$textBox.text(d.text()))};jQuery.fn.jplist.controls.BootstrapItemsPerPageDropdown=
function(b){return new a(b)};jQuery.fn.jplist.controlTypes["boot-items-per-page-dropdown"]={className:"BootstrapItemsPerPageDropdown",options:{},dropdown:!0}})();(function(){jQuery.fn.jplist.controls.BootstrapDropdownPaginationDTO=function(f){return{number:f}}})();
